...
export function initializeStorage(): void {
  ...
  if (!existingPass) localStorage.setItem(STORAGE_KEY_PASSCODE, TEACHER_PASSCODE_DEFAULT);
}
...
export async function findStudentByCode(code: string): Promise<StudentRecord | null> {
  ...
  const { data, error } = await supabase.from('students').select('*').ilike('id', clean).maybeSingle();
  if (error) { console.error('Error finding student:', error); return null; }
  if (!data) return null;
...
export function getRecentCodes(): string[] {
  if (typeof window === 'undefined') return [];
  try { const raw = localStorage.getItem(STORAGE_KEY_RECENT_CODES); return raw ? JSON.parse(raw) : []; }
  catch { return []; }
}
...
export function computeClassAverages(students: StudentRecord[]): TraitScores {
  ...
  if (students.length === 0) { SCALE_ORDER.forEach((k) => { avg[k] = { mean: 3.0, cat: 'M' }; }); return avg; }
  SCALE_ORDER.forEach((k) => {
    ...
    avg[k] = { mean, cat: calculateCategory(mean) };
  });
  return avg;
}
...
export function calculateStudyPairs(students: StudentRecord[]): PairMatch[] {
  ...
  for (let i = 0; i < students.length; i++) {
    for (let j = i + 1; j < students.length; j++) {
      const sA = students[i], sB = students[j];
      let distSq = 0;
      SCALE_ORDER.forEach((k) => { const diff = sA.scores[k].mean - sB.scores[k].mean; distSq += diff * diff; });
      ...
      if (similarityPct >= 80) { synergyType = 'Shared Momentum'; description = 'High intrinsic alignment in pace, structure, and communication rhythm.'; }
      else if ((sA.scores.structure.mean >= 4.0 && sB.scores.independence.mean >= 4.0) || (sB.scores.structure.mean >= 4.0 && sA.scores.independence.mean >= 4.0)) { synergyType = 'Balanced Synergy'; description = 'One student provides clear organization while the other accelerates autonomous inquiry.'; }
      else if ((sA.scores.practice.mean >= 4.2 && sB.scores.expression.mean >= 4.2) || (sB.scores.practice.mean >= 4.2 && sA.scores.expression.mean >= 4.2)) { synergyType = 'Creative Dynamic'; description = 'Strong synthesis between hands-on experiential building and articulate presentation.'; }
      else if ((sA.scores.stress.mean >= 4.2 && sB.scores.stress.mean <= 3.0) || (sB.scores.stress.mean >= 4.2 && sA.scores.stress.mean <= 3.0)) { synergyType = 'Peer Mentorship'; description = 'High resilience anchor offers calm grounding during complex problem sets.'; }
      matches.push({ studentA: sA, studentB: sB, similarityPct, synergyType, description });
    }
  }
...
export function exportCohortToCSV(students: StudentRecord[], classLabel: string): void {
  const header = ['Student Code', 'Full Name', 'Grade', 'Class', 'Archetype', ...SCALE_ORDER.map((k) => `${SCALES[k].label} (Score)`), ...SCALE_ORDER.map((k) => `${SCALES[k].label} (Level)`), 'Requested Support Reflection', 'Recorded Date'];
  ...
  const rows = students.map((s) => {
    ...
    return [s.id, s.name, s.grade, s.className, s.archetype?.name || determineArchetype(s.scores).name, ...scores, ...levels, s.reflection || '', dateStr].map(escapeCSV).join(',');
  });
  ...
  link.setAttribute('download', `wils-cohort-${classLabel.replace(/\s+/g, '-').toLowerCase()}.csv`);
